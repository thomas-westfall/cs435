...
#make athletic facilities portion of places csv 
#extract
col_list = ["Name","Borough","EditDate", "polygon","Status","ClosureType"]
df = pd.read_csv("../input/athfac.csv", usecols=col_list)
...
for index, row in df.iterrows():
    #print(row["EditDate"])
    d = datetime.strptime(row["EditDate"], "%m/%d/%y %H:%M")
    d.strftime("%I:%M %p")
...
#load
df.to_csv('../output/athfacnew2.csv', index=False)
